            ...
            try:
                ...
                try:
                    with st.spinner("生成特征影响解释..."):
                        ...
                        # 设置标题和标签
                        ax.set_title('特征对生存预测的影响', fontsize=15)
                        ax.set_xlabel('SHAP值 (正值=增加风险, 负值=降低风险)', fontsize=12)
                        ...
                        # 另一种方式：使用shap的summary_plot - 专门处理多维SHAP值
                        st.subheader("SHAP值分布图")
                        ...
                        try:
                            ...
                            if not isinstance(shap_values, list) and len(shap_values.shape) == 3:
                                ...
                                # 手动创建SHAP值摘要图，以确保中文显示正确
                                features_df_display = features_df.copy()
                                
                                # 获取特征重要性排序
                                feature_importance = pd.DataFrame({
                                    '特征': feature_names,
                                    'SHAP值': np.abs(shap_values_plot).mean(axis=0)
                                ...
                                ax.barh(y_pos, feature_importance['SHAP值'], align='center')
                                ax.set_yticks(y_pos)
                                ax.set_yticklabels(feature_importance['特征'])
                                ax.invert_yaxis()  # 最大值在顶部
                                ax.set_xlabel('平均|SHAP值|（特征重要性）')
                                ax.set_title('特征重要性（基于SHAP值）')
                            ...
                            st.warning(f"无法生成SHAP摘要图: {str(shap_plot_error)}")
                        
                        # 创建瀑布图 (Waterfall Plot) - 替代力图，更适合Streamlit环境
                        st.subheader("预测细节解释")
                        ...
                        try:
                            if not isinstance(shap_values, list) and len(shap_values.shape) == 3:
                                ...
                                feature_shap_df = pd.DataFrame({
                                    '特征': feature_names,
                                    'SHAP值': single_sample_shap
                                })
                                
                                # 按SHAP值的绝对值排序
                                feature_shap_df['绝对值'] = np.abs(feature_shap_df['SHAP值'])
                                feature_shap_df = feature_shap_df.sort_values('绝对值', ascending=False)
                                
                                # 创建一个自定义瀑布图
                                # 按特征影响排序（正向影响从高到低，负向影响从低到高）
                                feature_shap_df = feature_shap_df.sort_values('SHAP值', ascending=False)
                                ...
                                # 数据准备
                                features = feature_shap_df['特征'].tolist()
                                shap_values_list = feature_shap_df['SHAP值'].tolist()
                                ...
                                ax.set_title('特征贡献瀑布图')
                                ax.set_xlabel('预测的对数几率 (log-odds)')
                            ...
                            st.warning(f"无法生成瀑布图: {str(waterfall_error)}")
